...
class SmartAutomationWidget:
    """Tkinter widget for controlling smart automation with enhanced responsiveness and clarity."""
    
    def __init__(self, parent, automation: 'SmartAutomation', logger: 'AsyncLogger'):
        self.parent = parent
        self.automation = automation
        self.logger = logger  # Now accepts logger for direct use
        self.setup_ui()
    ...
    def setup_ui(self):
        """Setup the automation control UI with improved responsiveness."""
        # Main frame
        self.main_frame = ttk.LabelFrame(self.parent, text="🤖 Smart Automation Manager", padding=15)
        self.main_frame.pack(fill="both", expand=True, padx=10, pady=10)
        
        # Configure main frame rows/columns to be responsive
        self.main_frame.grid_rowconfigure(0, weight=0) # Profiles
        self.main_frame.grid_rowconfigure(1, weight=0) # Scheduling
        self.main_frame.grid_rowconfigure(2, weight=0) # Statistics
        self.main_frame.grid_rowconfigure(3, weight=1) # Task Activity (should expand)
        self.main_frame.grid_columnconfigure(0, weight=1)

        # ------------------------------------------------------------------
        # Quick Profiles Section (Row 0)
        # ------------------------------------------------------------------
        profiles_frame = ttk.LabelFrame(self.main_frame, text="Apply Common Automation Profiles", padding=10)
        profiles_frame.grid(row=0, column=0, sticky="ew", pady=10)
        
        button_frame = ttk.Frame(profiles_frame)
        button_frame.pack(fill="x")
        
        profiles = [
            ("🎮 Gaming", OptimizationProfile.GAMING, "danger-outline"),  # Danger for high impact
            ("💼 Work", OptimizationProfile.WORK, "info-outline"),
            ("🔒 Privacy", OptimizationProfile.PRIVACY, "warning-outline"),
            ("⚡ Performance", OptimizationProfile.PERFORMANCE, "primary-outline"),
            ("🔧 Maintenance", OptimizationProfile.MAINTENANCE, "secondary-outline")
        ]
        ...
        for i, (name, profile, style) in enumerate(profiles):
            ...
            button_frame.grid_columnconfigure(i, weight=1) # Make all columns stretch equally

        # ------------------------------------------------------------------
        # Scheduled Tasks Section (Row 1) - Improved Clarity
        # ------------------------------------------------------------------
        scheduling_container = ttk.Frame(self.main_frame)
        scheduling_container.grid(row=1, column=0, sticky="ew", pady=10)
        scheduling_container.grid_columnconfigure(0, weight=1)
        scheduling_container.grid_columnconfigure(1, weight=1)
        
        # Schedule Input Form (Left - Separate LabelFrame for clarity)
        schedule_form_frame = ttk.LabelFrame(scheduling_container, text="➕ Create New Scheduled Task", padding=10)
        schedule_form_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        
        # Configure form for proportional expansion
        schedule_form_frame.grid_columnconfigure(1, weight=1) 
        
        ttk.Label(schedule_form_frame, text="Task to Schedule:", font=("Segoe UI", 10, "bold")).grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.task_var = tk.StringVar()
        task_combo = ttk.Combobox(schedule_form_frame, textvariable=self.task_var, state="readonly", bootstyle="info")
        ...
        ttk.Label(schedule_form_frame, text="Recurrence:", font=("Segoe UI", 10, "bold")).grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.schedule_type_var = tk.StringVar(value="daily")
        schedule_combo = ttk.Combobox(schedule_form_frame, textvariable=self.schedule_type_var, state="readonly", width=10)
        schedule_combo['values'] = ['daily', 'weekly', 'interval']
        schedule_combo.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        
        ttk.Label(schedule_form_frame, text="Time/Interval:", font=("Segoe UI", 10, "bold")).grid(row=2, column=0, sticky="w", padx=5, pady=5)
        self.schedule_time_var = tk.StringVar(value="14:30")
        time_entry = ttk.Entry(schedule_form_frame, textvariable=self.schedule_time_var, width=10, bootstyle="info")
        time_entry.grid(row=2, column=1, padx=5, pady=5, sticky="ew")
        
        schedule_btn = tb.Button(schedule_form_frame, text="📅 Add Schedule", bootstyle="primary",
                               command=self.add_schedule)
        schedule_btn.grid(row=3, column=0, columnspan=2, padx=5, pady=10, sticky="ew")

        # Scheduled tasks list (Right)
        scheduled_tasks_list_frame = ttk.LabelFrame(scheduling_container, text="Current Scheduled Tasks", padding=10)
        scheduled_tasks_list_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
        scheduled_tasks_list_frame.grid_rowconfigure(0, weight=1)
        scheduled_tasks_list_frame.grid_columnconfigure(0, weight=1)
        
        columns = ("Task", "Schedule", "Status")
        self.scheduled_tree = ttk.Treeview(scheduled_tasks_list_frame, columns=columns, show="headings", height=4, bootstyle="striped")
        
        for col in columns:
            self.scheduled_tree.heading(col, text=col)
        
        # Adaptive Treeview Columns (Responsiveness)
        self.scheduled_tree.column("Task", width=1, stretch=True) 
        self.scheduled_tree.column("Schedule", width=120, stretch=False)
        self.scheduled_tree.column("Status", width=80, stretch=False)
        
        scheduled_scrollbar = ttk.Scrollbar(scheduled_tasks_list_frame, orient="vertical", command=self.scheduled_tree.yview)
        self.scheduled_tree.configure(yscrollcommand=scheduled_scrollbar.set)
        
        self.scheduled_tree.grid(row=0, column=0, sticky="nsew")
        scheduled_scrollbar.grid(row=0, column=1, sticky="ns")

        # ------------------------------------------------------------------
        # Statistics Section (Row 2) - Enhanced Clarity with Color-Coding
        # ------------------------------------------------------------------
        ...
        self.stats_label = ttk.Label(stats_frame, text="Loading statistics...", font=("Segoe UI", 11, "bold"))
        self.stats_label.pack(fill="x")
        
        # ------------------------------------------------------------------
        # Task Status Section (Row 3) - Logs (Expands)
        # ------------------------------------------------------------------
        status_frame = ttk.LabelFrame(self.main_frame, text="Recent Task Activity Log", padding=10)
        status_frame.grid(row=3, column=0, sticky="nsew", pady=10) # sticky="nsew" and weight=1 ensures expansion
        
        self.status_text = tk.Text(status_frame, height=8, wrap=tk.WORD, font=("Consolas", 9), background="#F4F4F4", borderwidth=0)
        ...
        # Update displays immediately and schedule recurring updates
    ...
    def on_task_complete(self, result: 'AutomationResult'):
        ...
        This improves responsiveness by not blocking the main thread.
        """
        # Safely schedule the update on the main thread
        self.parent.after(0, self._process_task_result, result)

    def _process_task_result(self, result: 'AutomationResult'):
        """Process the result on the main thread and update UI."""
        task = self.automation.tasks.get(result.task_id)
        if task:
            status_char = "✅" if result.success else "❌"
            log_level = LogLevel.SUCCESS if result.success else LogLevel.ERROR
            
            # Use the application logger for better visibility
            self.logger.log(
                f"{task.name} finished: {status_char} {result.message} (Exec time: {result.execution_time:.2f}s)",
                log_level
            )

            # Manually update the activity log
            self.update_recent_activity()
            self.update_statistics() # Update stats after a run

    def add_schedule(self):
        # ... (unchanged - relies on self.automation.schedule_task)
        ...
        if task_id and self.automation.schedule_task(task_id, schedule_type, schedule_time):
            ...
            messagebox.showerror("Error", "Failed to schedule task. Check time format (HH:MM or */MINUTES).")
        # ...
    
    def update_statistics(self):
        """Update automation statistics display with color-coded success rate for clarity."""
        try:
            stats = self.automation.get_task_statistics()
            rate = stats.get('success_rate', 0)

            # Determine color/style for success rate
            if rate >= 95:
                rate_style = "success"
                rate_color = "green"
            elif rate >= 70:
                rate_style = "warning"
                rate_color = "#F39C12"  # Yellow/Orange
            else:
                rate_style = "danger"
                rate_color = "red"
            
            stats_text = (
                f"Active Tasks: {stats['enabled_tasks']}/{stats['total_tasks']} | "
                f"Total Successes: {stats['total_successes']}"
            )

            # Update fixed label text
            self.stats_label.config(text=stats_text)

            # Create/update a separate label for the rate to apply color
            if not hasattr(self, 'rate_label'):
                self.rate_label = ttk.Label(self.stats_label.master, text="", font=("Segoe UI", 11, "bold"))
                self.rate_label.pack(pady=5, fill="x")

            self.rate_label.config(
                text=f"Recent Success Rate: {rate:.1f}%", 
    ...
    def update_scheduled_tasks(self):
        ...
        for schedule_id, scheduled_task in self.automation.scheduled_tasks.items():
            ...
            if task:
                ...
                schedule_info = f"{scheduled_task.schedule_type.title()} at {scheduled_task.schedule_time}"
                
                # Use a specific tag (style) for disabled items for clarity
                style_tag = "disabled" if not scheduled_task.enabled else ""
                self.scheduled_tree.insert("", "end", values=(task.name, schedule_info, status), tags=(style_tag,))

        self.scheduled_tree.tag_configure("disabled", foreground="gray", font=("Segoe UI", 9, "italic"))

    def update_recent_activity(self):
        """Update recent activity display by reading from the automation results history."""
        ...
        for result in recent_results:
            ...
            if task:
                ...
                display_msg = f"[{timestamp}] {status_char} Task '{task.name}' completed. Message: {result.message}\n"
                
                self.status_text.insert(tk.END, display_msg)
                
                # Apply color tagging for quick identification
                if result.success:
                    self.status_text.tag_add("success", f"{self.status_text.index(tk.END)} - {len(display_msg)}c", tk.END)
                else:
                    self.status_text.tag_add("error", f"{self.status_text.index(tk.END)} - {len(display_msg)}c", tk.END)

        # Configure tags (assuming these tags are defined in the main application's logger setup)
        self.status_text.tag_configure("success", foreground="#27AE60")
        self.status_text.tag_configure("error", foreground="#E74C3C")
        
        self.status_text.see(tk.END) # Auto-scroll to latest entry
...
class VirusTotalScanner:
    """Uses the VirusTotal API to scan files."""
    ...
    def scan_file(self, file_path: str, progress_callback: Optional[Callable] = None) -> List[VirusScanResult]:
        """Upload and scan a file using VirusTotal API"""
        if not self.api_key:
            if self.logger:
                self.logger.log("VirusTotal API Key is missing!", LogLevel.ERROR)
            return []
        ...
        try:
            ...
            if progress_callback:
                progress_callback(10, 100, "Calculating file hash...", file_path)

            file_hash = self.calculate_hash(file_path)

            # 2. Check if file already exists in VT database
            if progress_callback:
                progress_callback(30, 100, "Checking VirusTotal database...", "Hash: " + file_hash[:15] + "...")
            ...
            if response.status_code == 200:
                # File found, get results immediately
                data = response.json()
                analysis_stats = data['data']['attributes']['last_analysis_stats']
                if self.logger:
                    self.logger.log(f"File found in VT cache: {file_path}", LogLevel.INFO)

            elif response.status_code == 404:
                # File not found, upload it
                if progress_callback:
                    progress_callback(50, 100, "Uploading file for analysis...", "This may take a moment")

                # Check file size (limit is usually 32MB for standard endpoint)
                if os.path.getsize(file_path) > 32 * 1024 * 1024:
                    if self.logger:
                        self.logger.log("File too large for standard API upload (>32MB)", LogLevel.WARNING)
                    return []

                files = {"file": (os.path.basename(file_path), open(file_path, "rb"))}
                upload_res = requests.post(f"{self.base_url}/files", headers=headers, files=files)

                if upload_res.status_code == 200:
                    analysis_id = upload_res.json()['data']['id']
                    analysis_stats = self._poll_analysis(analysis_id, headers, progress_callback)
                else:
                    if self.logger:
                        self.logger.log(f"Upload failed: {upload_res.status_code} - {upload_res.text}", LogLevel.ERROR)

            else:
                if self.logger:
                    self.logger.log(f"API Error: {response.status_code} - {response.text}", LogLevel.ERROR)
            ...
            if analysis_stats:
                ...
                threat_name = f"Detections: {malicious}/{malicious + suspicious + harmless}"
                action = "Report Only"
                ...
                if progress_callback:
                    progress_callback(100, 100, "Analysis Complete", f"Malicious: {malicious}")

        except Exception as e:
            if self.logger:
                self.logger.log(f"VirusTotal Scan Error: {str(e)}", LogLevel.ERROR)
        finally:
            self.is_scanning = False

        return results

    def _poll_analysis(self, analysis_id, headers, progress_callback):
        """Poll the analysis endpoint until completion"""
        url = f"{self.base_url}/analyses/{analysis_id}"

        while self.is_scanning:
            if progress_callback:
                progress_callback(75, 100, "Waiting for analysis...", "Polling VirusTotal...")
            ...
            if resp.status_code == 200:
                ...
                if status == "completed":
                    ...
                    time.sleep(10) # Wait longer for queue
                else:
                    time.sleep(5)
            else:
                break
...
class EnhancedRegistryHelper:
    """
    Advanced helper for scanning Windows Registry and file system.
    Optimized for high-performance discovery and path validation.
    """
    ...
    @staticmethod
    def _get_msi_install_location(product_code: str) -> str:
        """Query Windows Installer database for install location of MSI package"""
        if not product_code:
            return ""
    
        try:
            # Clean the product code (remove braces if present)
            clean_code = product_code.strip('{}')
        
            # Try to read from Windows Installer registry
            # Path: HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\UserData\S-1-5-18\Products\{GUID}\InstallProperties

            # First, try the standard location
            try:
                key_path = f"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Installer\\UserData\\S-1-5-18\\Products\\{clean_code}\\InstallProperties"
            ...
            # Try alternative location (per-user installs)
            try:
                ...
                key_path = f"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Installer\\UserData\\{sid_string}\\Products\\{clean_code}\\InstallProperties"
                key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, key_path)
                try:
                    install_location = winreg.QueryValueEx(key, "InstallLocation")[0]
                    winreg.CloseKey(key)
            ...
            # Try using Windows COM API (most reliable)
            try:
                import win32com.client
                installer = win32com.client.Dispatch("WindowsInstaller.Installer")
            
                # Get install location property
                try:
                    install_location = installer.ProductInfo(product_code, "InstallLocation")
                    if install_location and os.path.exists(install_location):
                        return install_location
                except:
                    pass
    ...
    @staticmethod
    def _extract_msi_product_code(uninstall_str: str) -> str:
        """Extract MSI product code from uninstall string"""
        if not uninstall_str or "msiexec" not in uninstall_str.lower():
            return ""
    
        # Look for GUID pattern: {XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX}
        guid_pattern = r'\{[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}\}'
        match = re.search(guid_pattern, uninstall_str, re.IGNORECASE)
    ...
    def _extract_program_info(subkey) -> Optional['ProgramInfo']:
        """Extract detailed program information from registry with MSI support"""
        try:
            ...
            # ===== ENHANCED INSTALL LOCATION DETECTION =====
        
            # Method 1: Registry InstallLocation (15% success)
            try:
            ...
            # Method 2: MSI Database Query (handles 85% of programs!)
            if not install_location and uninstall_str and "msiexec" in uninstall_str.lower():
                product_code = EnhancedRegistryHelper._extract_msi_product_code(uninstall_str)
                if product_code:
                    install_location = EnhancedRegistryHelper._get_msi_install_location(product_code)
        
            # Method 3: Extract from UninstallString (non-MSI installers)
            if not install_location and uninstall_str and "msiexec" not in uninstall_str.lower():
                install_location = EnhancedRegistryHelper._extract_from_uninstall_string(uninstall_str)
        
            # Method 4: Extract from DisplayIcon
            if not install_location:
                try:
                    icon_path = winreg.QueryValueEx(subkey, "DisplayIcon")[0]
                    if icon_path:
                        install_location = EnhancedRegistryHelper._extract_from_icon_path(icon_path)
                except FileNotFoundError:
                    pass
        
            # Method 5: Search common directories (last resort)
            if not install_location and display_name:
                install_location = EnhancedRegistryHelper._search_install_directories(display_name)
    ...
    @staticmethod
    def _extract_location_from_uninstall_string(uninstall_str: str) -> str:
        """Extract installation path from uninstall string"""
        ...
        # Common patterns in uninstall strings
        # Example: "C:\Program Files\App\uninstall.exe"
        # We want: "C:\Program Files\App"
        
        try:
            # If it's an executable path, get the directory
            if os.path.isfile(uninstall_str):
                return os.path.dirname(uninstall_str)
            
            # Handle MsiExec paths like: MsiExec.exe /X{GUID}
            if "msiexec" in uninstall_str.lower():
                return ""
            
            # Handle paths with arguments
            # Example: "C:\Program Files\App\uninstall.exe" /S
            ...
            # Try to extract path before any common arguments
            for separator in [' /S', ' /SILENT', ' --uninstall', ' -uninstall', ' /X', ' /I']:
                if separator in uninstall_str:
                    path = uninstall_str.split(separator)[0].strip('"')
                    if os.path.isfile(path):
                        return os.path.dirname(path)
            
            # Last resort: if the string looks like a path
            if '\\' in uninstall_str:
                # Get the part before any space (likely arguments start)
                potential_path = uninstall_str.split(' ')[0].strip('"')
    ...
    @staticmethod
    def _extract_location_from_path(path: str) -> str:
        ...
        try:
            ...
            # Handle paths with comma-separated index (icon files)
            if ',' in path:
                path = path.split(',')[0]
    ...
    @staticmethod
    def _find_install_location_by_search(program_name: str) -> str:
        """Search common installation directories for the program"""
        if not program_name:
            return ""
        
        # Clean the program name for searching
        clean_name = re.sub(r'\s*\d+\.\d+.*$', '', program_name)  # Remove version
        clean_name = re.sub(r'\s*(x64|x86|64-bit|32-bit).*$', '', clean_name, flags=re.IGNORECASE)
        ...
        for base_path in search_paths:
            ...
            try:
                ...
                potential_path = base_path / program_name
                if potential_path.exists() and potential_path.is_dir():
                    return str(potential_path)
                
                # Look for partial match
                potential_path = base_path / clean_name
                if potential_path.exists() and potential_path.is_dir():
                    return str(potential_path)
                
                # Search subdirectories (limit depth to 1 for performance)
                for item in base_path.iterdir():
                    if item.is_dir():
                        # Case-insensitive partial match
                        if clean_name.lower() in item.name.lower():
                            return str(item)
            except (PermissionError, OSError):
                continue
    ...
    @staticmethod
    def _extract_from_uninstall_string(uninstall_string: str) -> str:
        """
        Cleans an uninstall string to extract the actual executable path or directory.
        Removes quotes, arguments, and handles common uninstaller patterns.
        """
        if not uninstall_string:
            return ""
        
        # 1. Remove quotes if the path is wrapped in them
        path = uninstall_string.strip()
        if path.startswith('"'):
            ...
            # 2. If no quotes, split by space and take the first part (likely the path)
            # This is risky if paths have spaces, but common for simple commands
            path = path.split(' /')[0].split(' -')[0].strip()

        # 3. If it points to an executable, get the directory
        if os.path.isfile(path) or path.lower().endswith(('.exe', '.msi', '.bat')):
            return str(Path(path).parent)
            
        # 4. If the path exists as a directory, return it
    ...
    @staticmethod
    def _search_install_directories(program_name: str) -> str:
        """
        High-performance heuristic search for installation paths.
        Uses regex fuzzy matching and environment variable resolution.
        """
        # Return cached result if available to boost performance
        if program_name in EnhancedRegistryHelper._directory_cache:
            return EnhancedRegistryHelper._directory_cache[program_name]
        ...
        # Clean and escape program name for regex
        clean_name = re.sub(r'[^\w\s-]', '', program_name).strip()
                ...
                with os.scandir(base_path) as entries:
                    for entry in entries:
                        if entry.is_dir() and pattern.search(entry.name):
                            path = entry.path
                            EnhancedRegistryHelper._directory_cache[program_name] = path
                            return path
            except (PermissionError, OSError):
                continue
    ...
    @classmethod
    def get_software_list(cls) -> List['ProgramInfo']:
        """
        Scans all major registry hives for installed software.
        """
        software = []
        hives = [
            (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall"),
                    ...
                    for i in range(winreg.QueryInfoKey(root_key)[0]):
                        try:
                            ...
                            with winreg.OpenKey(root_key, subkey_name) as subkey:
                                ...
                                location = get_val("InstallLocation")
                                if not location or not os.path.exists(location):
                                    # Use the fix: call the helper method
                                    location = cls._search_install_directories(name)

                                prog = ProgramInfo(
                                    name=name,
                                    uninstall_command=get_val("UninstallString"),
        ...
        # Sort alphabetically for better UI performance
...
class EnhancedPyUninstallXPro:
    def __init__(self):
        ...
        self.virus_scanner = VirusTotalScanner(api_key=VT_API_KEY, logger=self.logger)
    ...
    def _setup_enhanced_programs_tab(self):
        ...
        self.search_var.trace("w", self._filter_programs)
    ...
    def _filter_programs_debounced(self, *args):
        ...
        # Set delay to 1ms for instant reaction
        self._filter_timer = self.root.after(1, self._filter_programs)

    def _filter_programs(self, *args):
        """Filter programs and visually 'highlight' matches with brackets [ ]"""
        try:
            ...
            for prog in self.programs_data:
                ...
                name_match = not search_text or search_text in prog.name.lower()
                pub_match = not publisher or publisher == "All" or prog.publisher == publisher
                
                if name_match and pub_match:
                    # --- HIGHLIGHTING LOGIC ---
                    # If we are searching for something, wrap the match in brackets []
                    # Example: searching "a" in "Steam" -> "Ste[a]m"
                    display_name = prog.name
                    if search_text:
                        # Use regex to replace the search text with [search text]
                        # This preserves the original case (A vs a)
                        pattern = re.compile(re.escape(search_text), re.IGNORECASE)
                        display_name = pattern.sub(lambda m: f"[{m.group(0)}]", prog.name)
                    
                    # Insert with the modified display_name
                    self.programs_tree.insert(
                        "", "end",
                        values=(
                            display_name,  # Uses the "Highlighted" name
            ...
            # 4. Re-enable selection
            self.programs_tree.configure(selectmode='extended')
            
            # 5. Auto-select first result (optional, keeps UI responsive)
            children = self.programs_tree.get_children()
            if children:
                self.programs_tree.see(children[0])
    ...
    def _setup_virus_scanner_tab(self):
        """Setup virus scanner tab (VirusTotal Edition)"""
        ...
        ttk.Label(title_frame, text="🛡️ Cloud Virus Scanner (VirusTotal)",
                 font=("Segoe UI", 18, "bold")).pack(side="left")

        # Scanner controls (Replaces API frame)
        controls_frame = ttk.LabelFrame(frame, text="Select File for Scan", padding=15)
        controls_frame.pack(fill="x", padx=10, pady=10)
        ...
        ttk.Label(custom_scan_frame, text="File to Upload:",
        ...
        tb.Button(custom_scan_frame, text="Browse File", bootstyle="secondary-outline",
                 command=self._browse_file_scan_path).pack(side="left")
        ...
        self.virus_scan_status = ttk.Label(progress_labels_frame, text="Ready to scan (API Key loaded)",
                                         font=("Segoe UI", 11))
        ...
        # Results area
        results_frame = ttk.LabelFrame(frame, text="Scan Results (VirusTotal Detections)", padding=10)
        results_frame.pack(fill="both", expand=True, padx=10, pady=10)

        columns = ("File", "Detections", "Severity", "Action", "Path")
        ...
        virus_column_widths = {"File": 200, "Detections": 150, "Severity": 100, "Action": 100, "Path": 400}
        ...
        self.start_virus_scan_btn = tb.Button(action_frame, text="🚀 Upload & Scan",
                                            bootstyle="primary", width=20,
    ...
    def _setup_smart_automation_tab(self): 
        ...
        ttk.Label(title_frame, text="🤖 Smart Automation Manager", 
                font=("Segoe UI", 11, "bold")).pack(side="left")
        ...
        self.automation_status_label = ttk.Label(status_frame, text="Automation Status: Ready", 
                                          font=("Segoe UI", 7))
        self.automation_status_label.pack(side="top")
    
        # Automation controls
        controls_frame = ttk.LabelFrame(frame, text="Automation Controls", padding=15)
        controls_frame.pack(fill="x", padx=10, pady=10)
        ...
        ttk.Label(profile_frame, text="Optimization Profile:", 
                font=("Segoe UI", 10, "bold")).pack(side="left", padx=(0, 10))
    
        self.automation_profile_var = tk.StringVar(value="balanced")
        ttk.Radiobutton(profile_frame, text="Balanced", variable=self.automation_profile_var, 
                   value="balanced").pack(side="left", padx=10)
        ttk.Radiobutton(profile_frame, text="Performance", variable=self.automation_profile_var, 
                   value="performance").pack(side="left", padx=10)
        ttk.Radiobutton(profile_frame, text="Power Saver", variable=self.automation_profile_var, 
                   value="power_saver").pack(side="left", padx=10)
        ttk.Radiobutton(profile_frame, text="Custom", variable=self.automation_profile_var, 
        ...
        ttk.Label(options_frame, text="Automation Options:", 
             font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 5))
    
        # Checkboxes for automation features
        ...
        self.auto_defrag_var = tk.BooleanVar(value=False)
        self.auto_update_var = tk.BooleanVar(value=True)
    
        ttk.Checkbutton(checkbox_frame, text="Auto Cleanup", 
                   variable=self.auto_cleanup_var).pack(side="left", padx=10)
        ttk.Checkbutton(checkbox_frame, text="Auto Optimize", 
                   variable=self.auto_optimize_var).pack(side="left", padx=10)
        ttk.Checkbutton(checkbox_frame, text="Auto Defragment", 
                   variable=self.auto_defrag_var).pack(side="left", padx=10)
        ttk.Checkbutton(checkbox_frame, text="Auto Updates", 
                   variable=self.auto_update_var).pack(side="left", padx=10)
    
        # Schedule settings
        schedule_frame = ttk.Frame(controls_frame)
        schedule_frame.pack(fill="x", pady=10)
    
        ttk.Label(schedule_frame, text="Schedule:", 
             font=("Segoe UI", 10)).pack(side="left", padx=(0, 10))
        ...
        schedule_combo.pack(side="left", padx=(0, 10))
    
        ttk.Label(schedule_frame, text="Time:", 
             font=("Segoe UI", 10)).pack(side="left", padx=(20, 10))
    
        self.schedule_time_var = tk.StringVar(value="02:00")
        time_entry = ttk.Entry(schedule_frame, textvariable=self.schedule_time_var, 
                          width=10, font=("Segoe UI", 10))
        time_entry.pack(side="left")
    
        # Progress section
        progress_frame = ttk.LabelFrame(frame, text="Automation Progress", padding=15)
        progress_frame.pack(fill="x", padx=10, pady=10)
    
        self.automation_progress = ttk.Progressbar(progress_frame, length=500, mode="determinate")
        self.automation_progress.pack(pady=5)
    
        progress_labels_frame = ttk.Frame(progress_frame)
        progress_labels_frame.pack(fill="x", pady=5)
    
        self.automation_status = ttk.Label(progress_labels_frame, text="Ready to automate", 
                                     font=("Segoe UI", 10))
        self.automation_status.pack(side="left")
    
        self.automation_detail = ttk.Label(progress_labels_frame, text="", 
                                     font=("Segoe UI", 9), foreground="gray")
        self.automation_detail.pack(side="right")
    
        # Animation label
        self.automation_animation = ttk.Label(progress_frame, text="", font=("Segoe UI", 10))
        self.automation_animation.pack(pady=5)
        self.automation_animator = SmoothAnimationHandler(self.automation_animation)
    
        # Results area
        results_frame = ttk.LabelFrame(frame, text="Automation History", padding=10)
        results_frame.pack(fill="both", expand=True, padx=10, pady=10)
    
        # Results tree
        columns = ("Task", "Status", "Time", "Duration", "Details")
        self.automation_tree = ttk.Treeview(results_frame, columns=columns, show="tree headings", height=10)
    
        self.automation_tree.heading("#0", text="", anchor="w")
        self.automation_tree.column("#0", width=0, stretch=False)
    
        automation_column_widths = {"Task": 170, "Status": 100, "Time": 130, "Duration": 100, "Details": 350}
    
        for col in columns:
            self.automation_tree.heading(col, text=col, anchor="w")
            self.automation_tree.column(col, width=automation_column_widths[col], anchor="w")
    
        # Results scrollbars
        results_v_scrollbar = ttk.Scrollbar(results_frame, orient="vertical", 
                                            command=self.automation_tree.yview)
        results_h_scrollbar = ttk.Scrollbar(results_frame, orient="horizontal", 
                                            command=self.automation_tree.xview)
        self.automation_tree.configure(yscrollcommand=results_v_scrollbar.set, 
                                        xscrollcommand=results_h_scrollbar.set)
    
        self.automation_tree.pack(side="left", fill="both", expand=True)
        results_v_scrollbar.pack(side="right", fill="y")
        results_h_scrollbar.pack(side="bottom", fill="x")
        ...
        self.start_automation_btn = tb.Button(action_frame, text="🤖 Start Automation", 
                                            bootstyle="success", width=18,
                                            command=self.start_automation)
        self.start_automation_btn.pack(side="left", padx=10)
    
        self.apply_profile_btn = tb.Button(action_frame, text="✨ Apply Profile", 
                                        bootstyle="primary", width=15,
                                        command=self.apply_automation_profile)
        self.apply_profile_btn.pack(side="left", padx=10)
    
        self.save_config_btn = tb.Button(action_frame, text="💾 Save Config", 
                                    bootstyle="info", width=15,
                                    command=self.save_automation_config)
        self.save_config_btn.pack(side="left", padx=10)
    
        self.cancel_automation_btn = tb.Button(action_frame, text="❌ Cancel", 
                                         bootstyle="secondary", width=15,
                                         command=self.cancel_automation, state="disabled")
        self.cancel_automation_btn.pack(side="left", padx=10)
    ...
    def _update_automation_status(self):
        ...
        try:
            ...
            status = "Active" if hasattr(self, 'automation_running') and self.automation_running else "Ready"
            self.automation_status_label.config(text=f"Automation Status: {status}")
        except Exception as e:
            self.logger.log(f"Error updating automation status: {str(e)}", LogLevel.ERROR)

    def start_automation(self):
        """Start the automation process"""
        try:
            profile = self.automation_profile_var.get()
            self.logger.log(f"Starting automation with {profile} profile", LogLevel.INFO)
        
            # Disable start button, enable cancel
            self.start_automation_btn.config(state="disabled")
            self.cancel_automation_btn.config(state="normal")
        
            self.automation_running = True
            self.automation_status.config(text="Automation in progress...")
            self.automation_animator.start()
        
            # Run automation in thread
            thread = threading.Thread(target=self._run_automation_process, daemon=True)
            thread.start()
        
        except Exception as e:
            self.logger.log(f"Error starting automation: {str(e)}", LogLevel.ERROR)
            messagebox.showerror("Automation Error", f"Failed to start automation: {str(e)}")

    def _run_automation_process(self):
        """Execute the automation tasks"""
        try:
            tasks = []
            
            # Build task list based on selected options
            if self.auto_cleanup_var.get():
                tasks.append("System Cleanup")
            if self.auto_optimize_var.get():
                tasks.append("System Optimization")
            if self.auto_defrag_var.get():
                tasks.append("Disk Defragmentation")
            if self.auto_update_var.get():
                tasks.append("Update Check")
            ...
            for i, task in enumerate(tasks):
                ...
                self.automation_progress['value'] = progress
                self.automation_status.config(text=f"Running: {task}")
                self.automation_detail.config(text=f"Task {i+1} of {total_tasks}")
                
                # Simulate task execution (replace with actual)
                start_time = time.time()
                time.sleep(2)  # Placeholder
                duration = time.time() - start_time
                
                # Add to history
                self.root.after(0, self._add_automation_history, 
                            task, "Completed", time.strftime("%Y-%m-%d %H:%M:%S"), 
                            f"{duration:.2f}s", "Successfully executed")
            
            # Complete
            self.root.after(0, self._automation_complete)
            
        except Exception as e:
            self.logger.log(f"Automation error: {str(e)}", LogLevel.ERROR)
            self.root.after(0, self._automation_error, str(e))

    def _add_automation_history(self, task, status, time_str, duration, details):
        """Add entry to automation history"""
        self.automation_tree.insert("", "end", values=(task, status, time_str, duration, details))

    def _automation_complete(self):
        """Handle automation completion"""
        self.automation_running = False
        self.automation_progress['value'] = 100
        self.automation_status.config(text="Automation completed successfully")
        self.automation_detail.config(text="")
        self.automation_animator.stop()
    
        self.start_automation_btn.config(state="normal")
        self.cancel_automation_btn.config(state="disabled")
    
        self.logger.log("Automation completed successfully", LogLevel.SUCCESS)

    def _automation_error(self, error_msg):
        """Handle automation error"""
        self.automation_running = False
        self.automation_status.config(text=f"Automation failed: {error_msg}")
        self.automation_animator.stop()
    ...
    def cancel_automation(self):
        ...
        self.automation_running = False
        self.automation_status.config(text="Automation cancelled")
        self.automation_animator.stop()
    
        self.start_automation_btn.config(state="normal")
        self.cancel_automation_btn.config(state="disabled")
    
        self.logger.log("Automation cancelled by user", LogLevel.WARNING)

    def apply_automation_profile(self):
        """Apply the selected automation profile"""
        profile = self.automation_profile_var.get()
    
        # Set options based on profile
        if profile == "balanced":
            self.auto_cleanup_var.set(True)
            self.auto_optimize_var.set(True)
            self.auto_defrag_var.set(False)
            self.auto_update_var.set(True)
        elif profile == "performance":
            self.auto_cleanup_var.set(True)
            self.auto_optimize_var.set(True)
            self.auto_defrag_var.set(True)
            self.auto_update_var.set(True)
        ...
        self.logger.log(f"Applied {profile} profile", LogLevel.SUCCESS)
        messagebox.showinfo("Profile Applied", f"'{profile.replace('_', ' ').title()}' profile has been applied.")

    def save_automation_config(self):
        """Save the current automation configuration"""
        try:
            config = {
                'profile': self.automation_profile_var.get(),
                'auto_cleanup': self.auto_cleanup_var.get(),
                'auto_optimize': self.auto_optimize_var.get(),
                'auto_defrag': self.auto_defrag_var.get(),
                'auto_update': self.auto_update_var.get(),
                'schedule': self.schedule_var.get(),
                'schedule_time': self.schedule_time_var.get()
            }
        
            # Save to file (implement your own save logic, e.g., json dump)
            with open('automation_config.json', 'w') as f:
                json.dump(config, f, indent=4)
            self.logger.log("Automation configuration saved", LogLevel.SUCCESS)
            messagebox.showinfo("Configuration Saved", "Automation settings have been saved successfully.")
        
        except Exception as e:
            self.logger.log(f"Error saving configuration: {str(e)}", LogLevel.ERROR)
    ...
    def _browse_file_scan_path(self):
    ...
    def start_virus_scan(self):
        ...
        # API Key is now hardcoded in self.virus_scanner, no need to retrieve it here.

        file_path = self.custom_scan_path.get()
        ...
        self.virus_scan_animator.start('virus_scan', "Connecting to VirusTotal")

        # Assuming EnhancedProgressHandler is defined elsewhere
        progress_handler = EnhancedProgressHandler(
        ...
        def on_scan_complete(future):
            try:
                ...
                # Assuming _display_virus_scan_results is defined elsewhere
                ...
                # Assuming _set_virus_scan_buttons_state is defined elsewhere
                self.root.after(0, lambda: self._set_virus_scan_buttons_state(scanning=False))

        # Start background thread using the already initialized self.virus_scanner
        future = self.thread_pool.submit(
            self.virus_scanner.scan_file,
            file_path,
            update_progress
        )
        future.add_done_callback(on_scan_complete)
    ...
    def _display_virus_scan_results(self, scan_results: List[VirusScanResult], 
                                  progress_handler: EnhancedProgressHandler):
        """Display VirusTotal results in the tree"""
        # Clear existing items first (just in case)
        for item in self.virus_scan_tree.get_children():
            self.virus_scan_tree.delete(item)
        ...
        for result in scan_results:
            # Check if there are malicious detections
            is_malicious = "Severe" in result.severity or "High" in result.severity
            if is_malicious:
                threats_found += 1
            ...
            severity_icon = "⚪"
            if result.severity == "Severe": severity_icon = "🔴"
            elif result.severity == "Medium": severity_icon = "🟠"
            elif result.severity == "Low": severity_icon = "🟢"
            
            # Insert into Treeview (Matches columns: File, Detections, Severity, Action, Path)
            self.virus_scan_tree.insert("", "end", values=(
                file_name,
                result.threat_name, # This now contains "Detections: X/Y"
        ...
        # Log and Notify
        if threats_found > 0:
            self.logger.log(f"Scan completed: {threats_found} malicious files detected", LogLevel.SECURITY)
            messagebox.showwarning("VirusTotal Results", 
                                  f"Scan completed.\n{threats_found} files were flagged as malicious/suspicious.\n"
                                  "Please review the results carefully.")
        else:
            self.logger.log("Scan completed: File appears clean", LogLevel.SUCCESS)
            messagebox.showinfo("VirusTotal Results", "Scan completed. No engines detected threats.")
    ...
    def cancel_virus_scan(self):
        """Cancel the active VirusTotal scan"""
        # Signal the scanner loop to stop
        if hasattr(self, 'virus_scanner'):
            self.virus_scanner.is_scanning = False
        ...
        if self.logger:
            self.logger.log("VirusTotal scan cancelled by user", LogLevel.WARNING)
        
        self.virus_scan_status.config(text="Scan cancelled")

    def _set_virus_scan_buttons_state(self, scanning: bool = False):
        """Manage virus scan button states (VirusTotal Edition)"""
        # Start button is disabled when scanning, enabled otherwise
        self.start_virus_scan_btn.configure(state="disabled" if scanning else "normal")
        
        # Cancel button is enabled when scanning, disabled otherwise
    ...
    def on_close(self):
        ...
        try:
            ...
            self.root.destroy()
    def generate_virus_report(self, results):
        """Generates a professional single-file HTML/CSS report and opens it."""
        report_path = Path.home() / "Desktop" / f"ScanReport_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        ...
        <head>
            <meta charset="UTF-8">
            <title>PyUninstallX - Scan Report</title>
            <style>
                body {{ font-family: 'Segoe UI', sans-serif; background: #f8f9fa; padding: 20px; }}
                .card {{ background: white; border-radius: 10px; shadow: 0 4px 6px rgba(0,0,0,0.1); padding: 20px; max-width: 900px; margin: auto; }}
        ...
        <body>
            <div class="card">
                ...
                <table>
                    <thead>
                        <tr><th>File Name</th><th>Status</th><th>Detections</th></tr>
                    </thead>
                    <tbody>
        """
    
        for item in results:
            status_class = "threat" if item.get('positives', 0) > 0 else "clean"
            html_content += f"""
                <tr>
                    <td>{item.get('file', 'Unknown')}</td>
                    <td class="{status_class}">{"Malicious" if status_class == "threat" else "Clean"}</td>
                    <td>{item.get('positives', 0)} / {item.get('total', 0)}</td>
                </tr>
            """
        ...
        try:
            ...
            self.logger.log(f"Report opened: {report_path}", LogLevel.SUCCESS)
        except Exception as e:
            self.logger.log(f"Failed to create report: {e}", LogLevel.ERROR)
